...
pub trait TribufuGeneratedApi: Send + Sync {
    ...
    async fn generate_flake_id<'amount>(&self, amount: Option<i32>) -> Result<Vec<i64>, Error<GenerateFlakeIdError>>;

    /// GET /v1/utils/flake/{timestamp}
    ///
    /// <b>🔒 Required permissions:</b> <code>tribufu.utils.generate.flake.timestamp</code>
    async fn generate_flake_id_from_timestamp<'timestamp, 'amount>(&self, timestamp: i64, amount: Option<i32>) -> Result<Vec<i64>, Error<GenerateFlakeIdFromTimestampError>>;
    ...
    async fn generate_uuid<'version, 'amount>(&self, version: Option<i32>, amount: Option<i32>) -> Result<Vec<String>, Error<GenerateUuidError>>;
    ...
    async fn get_group_by_uuid<'uuid>(&self, uuid: &'uuid str) -> Result<models::Group, Error<GetGroupByUuidError>>;
    ...
    async fn get_user_accounts<'id>(&self, id: &'id str) -> Result<Vec<models::ExternalAccount>, Error<GetUserAccountsError>>;
    ...
    async fn get_user_profile_by_uuid<'uuid>(&self, uuid: &'uuid str) -> Result<models::Profile, Error<GetUserProfileByUuidError>>;

    /// GET /v1/users/uuid/{uuid}
    ///
    /// <b>🔒 Required permissions:</b> <code>tribufu.community.profile.get.uuid</code>
    async fn get_user_profile_by_uuid_v1<'uuid>(&self, uuid: &'uuid str) -> Result<models::Profile, Error<GetUserProfileByUuidV1Error>>;
...
impl TribufuGeneratedApi for TribufuGeneratedApiClient {
    ...
    async fn generate_flake_id<'amount>(&self, amount: Option<i32>) -> Result<Vec<i64>, Error<GenerateFlakeIdError>> {
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            match local_var_content_type {
                ...
                ContentType::Unsupported(local_var_unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{local_var_unknown_type}` content type response that cannot be converted to `Vec&lt;i64&gt;`")))),
            }
        } else {
    ...
    async fn generate_flake_id_from_timestamp<'timestamp, 'amount>(&self, timestamp: i64, amount: Option<i32>) -> Result<Vec<i64>, Error<GenerateFlakeIdFromTimestampError>> {
        ...
        let local_var_uri_str = format!("{}/v1/utils/flake/{timestamp}", local_var_configuration.base_path, timestamp=timestamp);
        let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    async fn get_user_accounts<'id>(&self, id: &'id str) -> Result<Vec<models::ExternalAccount>, Error<GetUserAccountsError>> {
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            match local_var_content_type {
                ContentType::Json => serde_json::from_str(&local_var_content).map_err(Error::from),
                ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::ExternalAccount&gt;`"))),
                ContentType::Unsupported(local_var_unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{local_var_unknown_type}` content type response that cannot be converted to `Vec&lt;models::ExternalAccount&gt;`")))),
            }
        } else {
